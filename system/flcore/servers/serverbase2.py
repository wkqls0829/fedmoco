...
class Server2(object):
    ...
    def select_clients(self):
        ...
        self.selected_client_ids = list(np.random.choice(range(self.num_clients), join_clients, replace=False))
        selected_clients = [self.clients[i] for i in self.selected_client_ids]
    ...
    def receive_models(self):
        ...
        tot_samples = 0
        for client_id, client in zip(self.selected_client_ids, self.selected_clients):
            try:
            ...
            if client_time_cost <= self.time_threthold:
                ...
                self.client_weights[client_id] = client.train_samples
                self.client_models[client_id] = client.model
        for i in self.selected_client_ids:
            self.client_weights[i] /= tot_samples

        if self.static_weight:
            for i, ids in enumerate(self.selected_client_ids):
                if i == 0:
                    self.client_weights[ids] = self.static_weight
                else:
                    self.client_weights[ids] = (1 - self.static_weight) / (self.join_clients - 1)
    ...
    def save_global_model(self):
        ...
        model_path = os.path.join(model_path, self.file_name + ".pt")
